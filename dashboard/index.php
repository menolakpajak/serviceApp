...
$chart = [];
for ($x = $i; $x < $a; $x++) {
	array_push($chart, count(data("SELECT * FROM data WHERE date LIKE '$monthc[$x]' ")));
}
$chart = implode(',', $chart);


$canon = array_filter($all, function ($item) {
	return $item['tipe'] === 'canon';
});
$nikon = array_filter($all, function ($item) {
	return $item['tipe'] === 'nikon';
});
$fuji = array_filter($all, function ($item) {
	return $item['tipe'] === 'fujifilm';
});
$sony = array_filter($all, function ($item) {
	return $item['tipe'] === 'sony';
});
$gopro = array_filter($all, function ($item) {
	return $item['tipe'] === 'gopro';
});
$godox = array_filter($all, function ($item) {
	return $item['tipe'] === 'godox';
});
$dji = array_filter($all, function ($item) {
	return $item['tipe'] === 'dji';
});
$other = array_filter($all, function ($item) {
	return $item['tipe'] === 'other';
});

if (!empty($canon)) {
	$per_canon = round(count($canon) / count($all) * 100);
} else {
	$per_canon = 0;
}
if (!empty($nikon)) {
	$per_nikon = round(count($nikon) / count($all) * 100);
} else {
	$per_nikon = 0;
}
if (!empty($fuji)) {
	$per_fuji = round(count($fuji) / count($all) * 100);
} else {
	$per_fuji = 0;
}
if (!empty($sony)) {
	$per_sony = round(count($sony) / count($all) * 100);
} else {
	$per_sony = 0;
}
if (!empty($gopro)) {
	$per_gopro = round(count($gopro) / count($all) * 100);
} else {
	$per_gopro = 0;
}
if (!empty($godox)) {
	$per_godox = round(count($godox) / count($all) * 100);
} else {
	$per_godox = 0;
}
if (!empty($dji)) {
	$per_dji = round(count($dji) / count($all) * 100);
} else {
	$per_dji = 0;
}
if (!empty($other)) {
	$per_other = round(count($other) / count($all) * 100);
} else {
	$per_other = 0;
}
		...
		<div class="panel panel-container">
			<div class="row">
				<div class="col-xs-6 col-md-3 col-lg-3 no-padding">
					<div class="panel panel-teal panel-widget border-right" onclick="direct('../order-new')">
						<div class="row no-padding"><em class="fa fa-xl fa-plus color-blue"></em>
							<div class="large"><?php if (isset($new)) {
								echo count($new);
							} else {
								echo 0;
							} ?></div>
				...
				<div class="col-xs-6 col-md-3 col-lg-3 no-padding">
					<div class="panel panel-blue panel-widget border-right" onclick="direct('../order-proses')">
						<div class="row no-padding"><em class="fa fa-xl fa-cogs color-orange"></em>
							<div class="large"><?php if (isset($proses)) {
								echo count($proses);
							} else {
								echo 0;
							} ?></div>
				...
				<div class="col-xs-6 col-md-3 col-lg-3 no-padding">
					<div class="panel panel-orange panel-widget border-right" onclick="direct('../order-done')">
						<div class="row no-padding"><em class="fa fa-xl fa-check color-green"></em>
							<div class="large"><?php if (isset($done)) {
								echo count($done);
							} else {
								echo 0;
							} ?></div>
				...
				<div class="col-xs-6 col-md-3 col-lg-3 no-padding">
					<div class="panel panel-red panel-widget border-right" onclick="direct('../order-abort')">
						<div class="row no-padding"><em class="fa fa-xl fa-times color-red"></em>
							<div class="large"><?php if (isset($abort)) {
								echo count($abort);
							} else {
								echo 0;
							} ?></div>
