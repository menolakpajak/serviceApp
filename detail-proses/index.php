...
if(empty($data)){
	header('Location: ../detail-pickup/?id='.$id);
	exit();
}
$data = $data[0];
...
if($data['status'] == 'new'){
	header('Location: ../detail-new/?id='.$id); die;}
if($data['status'] == 'done'){
	header('Location: ../detail-done/?id='.$id); die;}
if($data['status'] == 'abort'){
	header('Location: ../detail-abort/?id='.$id); die;}
...
$other = ucfirst($data2['other']);
	...
	if($data2['check_kamera'] == 'on'){$kamera = "";}
	if($data2['check_lensa'] == 'on'){$lensa = "";}
	if($data2['check_battery'] == 'on'){$battery = "";}
	if($data2['check_memory'] == 'on'){$memory = "";}
	if($data2['check_strap'] == 'on'){$strap = "";}
	if($data2['check_bodycap'] == 'on'){$bodyCap = "";}
	if($data2['check_lenscap'] == 'on'){$lensCap = "";}
	if($data2['check_filter'] == 'on'){$filter = "";}
...
<!-- nav bar -->
		...
		<div class="row">	
			<div class="col-lg-12" >
				<div style="display: flex; flex-direction:row;">
					<h1 style="font-weight: 500;" class="page-header color-orange"><?= ucfirst($data['status']) ; ?> </h1>
					<i style="font-size: x-large; margin-top:8px;" class="fa fa-cogs color-orange" aria-hidden="true"></i>
				</div>
	...
	<div id="ajax-update">
					<!-- PROSES INFO -->
				<?php if($data['acc'] == 'on'){$acc = 'acc';}else{$acc = '';} ?>
					<div id="proses_info" style="border:1px solid #ffa500b3 !important;" class="<?= $acc ; ?>">
						...
						<div class="action-grup">
							...
							<?php else: ?>
								<button class="btn btn-success" type="button" onclick="cekInvoice('<?= $data['invoice']; ?>','<?= $data['no_spk']; ?>')">Invoice</button>
					...
					<div id="action" >
						<div class="title_fill">
							...
							<div class="action action2">
								<?php if($akses == 'master') :?>
								<div class="action-grup">
									<a style="margin-bottom: 5px;" href="javascript:void(0)" class="btn btn-primary" onclick="editProses()"> Edit </a>
									<a href="javascript:void(0)" class="btn btn-danger" onclick="deleteData('proses','<?= $id ; ?>')"> Delete </a>
								</div>
								<?php endif; ?>
								<div class="action-grup">
									<a style="margin-bottom: 5px;" href="javascript:void(0)" class="btn btn-success" onclick="done('<?= $id ; ?>')"> Next to Done </a>
									<a href="javascript:void(0)" class="btn btn-danger" onclick="abort('<?= $id ; ?>')"> Next to Abort </a>
								</div>
								<a id="btn-update" style="text-align: center;height:fit-content; margin-right:5px;" href="javascript:void(0)" class="btn btn-dark" onclick="nextUpdate()"> Update </a>
								<a style="text-align: center;height:fit-content" href="../print/?id=<?= $id ; ?>" class="btn btn-dark" target="_blank">Receipt</a>
							</div>
