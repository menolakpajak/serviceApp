...
if ($data['status'] == 'done') {
	header('Location: ../detail-done/?id=' . $id);
	die;
}
if ($data['status'] == 'abort') {
	header('Location: ../detail-abort/?id=' . $id);
	die;
}
							...
							<form action="tes.php" method="post">
								...
								<div id="ajax-update">
									...
									<div id="action">
										...
										<div class="action">
											<?php if ($akses == 'master'): ?>
												<a href="javascript:void(0)" class="btn btn-primary mt-1" onclick="editProses()"> Edit </a>
												<a href="javascript:void(0)" class="btn btn-danger mt-1" onclick="deleteData('proses','<?= $id; ?>')"> Delete </a>
											<?php endif; ?>
											<a href="javascript:void(0)" class="btn btn-success mt-1" onclick="done('<?= $id; ?>')"> Next to Done </a>
											<a href="javascript:void(0)" class="btn btn-danger mt-1" onclick="abort('<?= $id; ?>')"> Next to Abort </a>
											<a href="../duplicateOrder/?id=<?= $id; ?>" class="btn btn-info mt-1">Duplicate</a>
											<a id="btn-update" href="javascript:void(0)" class="btn btn-dark mt-1" onclick="nextUpdate()"> Update </a>
