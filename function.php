...
function input($order){
    ...
    if(empty($order['date'])){
        $date = $datetime;
    }else{ $date = date('Y-m-d H:i',strtotime($order['date'])); }
...
function editNew($order){
    ...
    if($data['nama'] != $order['nama']){array_push($log['detail'],'Nama : '. $data['nama'] . ' => '. $order['nama']);}
    if($data['wa'] != $order['wa']){array_push($log['detail'],'No wa : '. $data['wa'] . ' => '. $order['wa']);}
    if($data['no_tlp'] != $order['no_tlp']){array_push($log['detail'],'No tlp : '. $data['no_tlp'] . ' => '. $order['no_tlp']);}
    if($data['alamat'] != $order['alamat']){array_push($log['detail'],'Alamat : '. $data['alamat'] . ' => '. $order['alamat']);}
    if($data['tipe'] != $order['tipe_unit']){array_push($log['detail'],'Tipe Unit : '. $data['tipe'] . ' => '. $order['tipe_unit']);}
    if($data['unit'] != $order['unit']){array_push($log['detail'],'Unit : '. $data['unit'] . ' => '. $order['unit']);}
    if($data['sn'] != $order['serial_number']){array_push($log['detail'],'Serial Number : '. $data['sn'] . ' => '. $order['serial_number']);}
    if($data['counter'] != $order['counter']){array_push($log['detail'],'Counter : '. $data['counter'] . ' => '. $order['counter']);}
    if($data['pin'] != $order['pin']){array_push($log['detail'],'Pin : '. $data['pin'] . ' => '. $order['pin']);}
    if($data['note'] != $order['note']){array_push($log['detail'],'Note : '. $data['note'] . ' => '. $order['note']);}

    if($data2['check_kamera'] != $order['check_kamera']){
        if($data2['check_kamera'] === 'on'){$from = 'check';}else{$from = 'uncheck';}
        if($order['check_kamera'] === 'on'){$to = 'check';}else{$to = 'uncheck';}
        array_push($log['detail'],'Kamera : '. $from . ' => '. $to);
    }
    if($data2['check_lensa'] != $order['check_lensa']){
        if($data2['check_lensa'] === 'on'){$from = 'check';}else{$from = 'uncheck';}
        if($order['check_lensa'] === 'on'){$to = 'check';}else{$to = 'uncheck';}
        array_push($log['detail'],'Lensa : '. $from . ' => '. $to);
    }
    if($data2['check_battery'] != $order['check_battery']){
        if($data2['check_battery'] === 'on'){$from = 'check';}else{$from = 'uncheck';}
        if($order['check_battery'] === 'on'){$to = 'check';}else{$to = 'uncheck';}
        array_push($log['detail'],'Battery : '. $from . ' => '. $to);
    }
    if($data2['check_memory'] != $order['check_memory']){
        if($data2['check_memory'] === 'on'){$from = 'check';}else{$from = 'uncheck';}
        if($order['check_memory'] === 'on'){$to = 'check';}else{$to = 'uncheck';}
        array_push($log['detail'],'Memory : '. $from . ' => '. $to);
    }
    if($data2['check_strap'] != $order['check_strap']){
        if($data2['check_strap'] === 'on'){$from = 'check';}else{$from = 'uncheck';}
        if($order['check_strap'] === 'on'){$to = 'check';}else{$to = 'uncheck';}
        array_push($log['detail'],'Strap : '. $from . ' => '. $to);
    }
    if($data2['check_bodycap'] != $order['check_bodycap']){
        if($data2['check_bodycap'] === 'on'){$from = 'check';}else{$from = 'uncheck';}
        if($order['check_bodycap'] === 'on'){$to = 'check';}else{$to = 'uncheck';}
        array_push($log['detail'],'Bodycap : '. $from . ' => '. $to);
    }
    if($data2['check_lenscap'] != $order['check_lenscap']){
        if($data2['check_lenscap'] === 'on'){$from = 'check';}else{$from = 'uncheck';}
        if($order['check_lenscap'] === 'on'){$to = 'check';}else{$to = 'uncheck';}
        array_push($log['detail'],'Lenscap : '. $from . ' => '. $to);
    }
    if($data2['check_filter'] != $order['check_filter']){
        if($data2['check_filter'] === 'on'){$from = 'check';}else{$from = 'uncheck';}
        if($order['check_filter'] === 'on'){$to = 'check';}else{$to = 'uncheck';}
        array_push($log['detail'],'filter : '. $from . ' => '. $to);
    }
    if($data2['check_kamera_info'] != $order['check_kamera_info']){array_push($log['detail'],'Kamera info : '. $data2['check_kamera_info'] . ' => '. $order['check_kamera_info']);}
    if($data2['check_lensa_info'] != $order['check_lensa_info']){array_push($log['detail'],'Lensa info : '. $data2['check_lensa_info'] . ' => '. $order['check_lensa_info']);}
    if($data2['check_battery_info'] != $order['check_battery_info']){array_push($log['detail'],'Battery info : '. $data2['check_battery_info'] . ' => '. $order['check_battery_info']);}
    if($data2['check_memory_info'] != $order['check_memory_info']){array_push($log['detail'],'Memory info : '. $data2['check_memory_info'] . ' => '. $order['check_memory_info']);}
    if($data2['check_strap_info'] != $order['check_strap_info']){array_push($log['detail'],'Strap info : '. $data2['check_strap_info'] . ' => '. $order['check_strap_info']);}
    if($data2['check_bodycap_info'] != $order['check_bodycap_info']){array_push($log['detail'],'Bodycap info : '. $data2['check_bodycap_info'] . ' => '. $order['check_bodycap_info']);}
    if($data2['check_lenscap_info'] != $order['check_lenscap_info']){array_push($log['detail'],'Lenscap info : '. $data2['check_lenscap_info'] . ' => '. $order['check_lenscap_info']);}
    if($data2['check_filter_info'] != $order['check_filter_info']){array_push($log['detail'],'Filter info : '. $data2['check_filter_info'] . ' => '. $order['check_filter_info']);}

    if($data2['other'] != $order['other']){array_push($log['detail'],'Other : '. $data2['other'] . ' => '. $order['other']);}

    if($data['error'] != $order['error']){array_push($log['detail'],'Kerusakan : '. $data['error'] . ' => '. $order['error']);}
...
function editProses($order){
    ...
    }else{$date_proses = $data['date_proses'];}
    if(!empty($order['date_update'])){
        ...
        }else{$date_update = 'date_update = '.$data['date_update'].',';}
    }else{
        $date_update = '';
    }
    ...
    if($data['pin'] != $order['pin']){array_push($log['detail'],'Pin : '. $data['pin'] . ' => '. $order['pin']);}
    if($data['note'] != $order['note']){array_push($log['detail'],'Note : '. $data['note'] . ' => '. $order['note']);}
    if($data['service_at'] != $service_at){array_push($log['detail'],'Service At : '. $data['service_at'] . ' => '. $service_at);}
    if($data['result'] != $order['result']){array_push($log['detail'],'Hasil Pengecekan : '. $data['result'] . ' => '. $order['result']);}
    if($data['cost'] != $order['cost']){array_push($log['detail'],'Estimasi Biaya : '. $data['cost'] . ' => '. $order['cost']);}
    if($data['acc'] != $order['acc']){array_push($log['detail'],'ACC : '. $data['acc'] . ' => '. $order['acc']);}
...
function editDone($order){
    ...
    if($data['date_update'] != $order['date_update']){
        array_push($log['detail'],'Date Update : '. $data['date_update'] . ' => '. $order['date_update']);
        $date_update = date('Y-m-d H:i:s',strtotime($order['date_update']));
    }else{$date_update = $data['date_update'];}
    ...
    }else{$date_finish = $data['date_finish'];}
...
function inputNewUser($order){
    ...
    if(empty($order['kode'])){die('kolom KODE USER tidak boleh kosong !');}
    if(empty($order['akses'])){die('kolom AKSES Belum Dipilih !');}
    if(empty($order['username'])){die('kolom USERNAME tidak boleh kosong !');}
    if(empty($order['counter'])){die('kolom COUNTER tidak boleh kosong !');}
